...
export function RegistrationForm({ onSuccess }: RegistrationFormProps) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(settingsRef, (snapshot) => {
      ...
      console.error("Error loading registration form settings:", error);
    });
  ...
  const validateStep = async (step: number): Promise<boolean> => {
    ...
    switch (step) {
      ...
      case 4: // Room & Tag Selection
        // Check if room and tag are selected based on settings
        if (roomRequired && !selectedRoomId) {
          return false;
        }
        if (tagRequired && !selectedTagId) {
          return false;
        }
        return true;
  ...
  const handleNext = async () => {
    ...
    if (isValid && currentStep < totalSteps) {
      ...
      if (currentStep === 4) {
        if (roomRequired && !selectedRoomId && tagRequired && !selectedTagId) {
          errorMessage = "Please select both a room and a tag before proceeding.";
        } else if (roomRequired && !selectedRoomId) {
          errorMessage = "Please select a room before proceeding.";
        } else if (tagRequired && !selectedTagId) {
                        ...
                        <div>
                          <FormItem>
                            <FormLabel className="flex items-center gap-2">
                              ...
                              Select Room {roomRequired && <span className="text-red-500">*</span>}
                              {!roomRequired && <span className="text-sm text-muted-foreground">(Optional)</span>}
                            </FormLabel>
                            <Select 
                            ...
                            {roomRequired && !selectedRoomId && availableRooms.length > 0 && (
                      ...
                      <div>
                        <FormItem>
                          <FormLabel className="flex items-center gap-2">
                            ...
                            Select Tag {tagRequired && <span className="text-red-500">*</span>}
                            {!tagRequired && <span className="text-sm text-muted-foreground">(Optional)</span>}
                          </FormLabel>
                          <Select 
                          ...
                          {tagRequired && !selectedTagId && availableTags.length > 0 && (
                            <p className="text-sm text-red-600 dark:text-red-400 mt-1">
