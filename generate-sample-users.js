...
// Sample Nigerian first names
const FIRST_NAMES = [
  "Chukwuemeka", "Oluwaseun", "Adebayo", "Chinonso", "Ikenna", "Oluwatobi", "Emeka", "Chinedu",
  "Adeyemi", "Ifeoma", "Ngozi", "Amina", "Fatima", "Hassan", "Ibrahim", "Musa",
  "Aisha", "Zainab", "Maryam", "Aminu", "Yusuf", "Mohammed", "Abdullahi", "Sani",
  "Blessing", "Gift", "Peace", "Joy", "Faith", "Hope", "Grace", "Mercy",
  "David", "John", "Michael", "James", "Peter", "Paul", "Daniel", "Samuel"
];

// Sample Nigerian surnames
const SURNAMES = [
  "Adebayo", "Okafor", "Nwankwo", "Okoro", "Eze", "Obi", "Nwosu", "Iwu",
  "Mohammed", "Ibrahim", "Hassan", "Aliyu", "Bello", "Yusuf", "Sani", "Aminu",
  "Adeyemi", "Oluwaseun", "Ogunleye", "Adekunle", "Oladipo", "Adeniyi", "Ogunjobi", "Adeleke",
  "Okonkwo", "Nwosu", "Eze", "Okafor", "Nwankwo", "Okoro", "Iwu", "Obi",
  "Johnson", "Williams", "Brown", "Davis", "Miller", "Wilson", "Moore", "Taylor"
];

// Sample middle names (optional)
const MIDDLE_NAMES = [
  "Chukwu", "Oluwa", "Ade", "Ngozi", "Ifeoma", "Amina", "Fatima", "Maryam",
  "Emeka", "Chinedu", "Ikenna", "Chinonso", "Oluwatobi", "Adebayo", "Adeyemi", "",
  "David", "John", "Michael", "James", "Peter", "Paul", "Daniel", ""
];
...
function generateEmail(firstName, surname) {
  ...
  return `${firstName.toLowerCase()}.${surname.toLowerCase()}${number}@${domain}`;
}
...
for (let i = 0; i < 50; i++) {
  ...
  const middleName = Math.random() > 0.3 ? MIDDLE_NAMES[Math.floor(Math.random() * MIDDLE_NAMES.length)] : "";
  const gender = Math.random() > 0.5 ? "Male" : "Female";
  const state = NIGERIAN_STATES[Math.floor(Math.random() * NIGERIAN_STATES.length)];
  const lgas = NIGERIAN_LGAS[state] || ["Default LGA"];
  const lga = lgas[Math.floor(Math.random() * lgas.length)];
  
  // Some users will have email and NIN, some won't (to test optional fields)
  const hasEmail = Math.random() > 0.2; // 80% have email
  const hasNIN = Math.random() > 0.3; // 70% have NIN
  const isVip = Math.random() > 0.9; // 10% are VIP
  ...
  users.push({
    ...
    'Gender': gender,
    'Phone': generatePhone(),
    'Email': hasEmail ? generateEmail(firstName, surname) : '',
    'NIN': hasNIN ? generateNIN() : '',
    'State of Origin': state,
    'LGA': lga,
    'Room Number': '', // Leave empty for auto-assignment
    'Tag Number': '', // Leave empty for auto-assignment
    'VIP': isVip ? 'Yes' : ''
  });
}
...
// Save file
const fileName = 'sample_users_50_full_details.xlsx';
XLSX.writeFile(workbook, fileName);

console.log(`✅ Generated ${fileName} with 50 users' full details!`);
console.log(`📊 File saved in: ${process.cwd()}/${fileName}`);
